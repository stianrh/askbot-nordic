...
class Command(BaseCommand):
    ...
    def import_threads(self, everyone, admin):
        from askbot.models import question, user
        ...
        for ed_post in ProgressBar(ed_posts.iterator(), count, message):
            ...
            ab_thread.language_code = self.LANGUAGE
    ...
    def import_answers(self, everyone, admin):
        from askbot.models import post, user
    ...
    def import_comments(self, everyone, admin):
        from askbot.models import post, user
        ...
        for ed_comment in ProgressBar(ed_comments.iterator(), count, message):
            ...
            try:
                author = user.User.objects.get(id=ed_comment.user_id)
                ab_post.author = author
            except:
    ...
    def import_attachments(self):
        from askbot.models import post, Attachment
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            admin = user.User.objects.filter(is_staff=True)[0]

            self.import_threads(everyone, admin)
            self.import_questions(everyone, admin)
            self.import_answers(everyone, admin)
            self.import_comments(everyone, admin)
            self.import_attachments()
            self.import_private_messages()
